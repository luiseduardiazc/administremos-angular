...
export class ResidenteService {
  ...
  generarPago (selected: Factura[], fileToUpload: File): Observable<any> {
    ...
    const valorNeto: Number =
      selected[0].valAdministracion.valueOf() + selected[0].cuotaExtra.valueOf()
  ...
  getPagosHistoricos (
    fechaInicio: Date,
    fechaFin: Date
  ): Observable<PagosHistoricoResidente> {
    ...
    let httpParams = new HttpParams()
      ...
      .set('fechaFin', fecFin)
    return this.http.get<PagosHistoricoResidente>(
      `${base_url}/pagos/residente`,
      { params: httpParams }
    )
  }
